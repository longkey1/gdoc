...
func initConfig() {
	if cfgFile != "" {
		...
		viper.AddConfigPath(filepath.Join(home, ".config/gdoc"))
...
func GetConfig() *gdoc.Config {
	if config == nil {
		cobra.CheckErr(fmt.Errorf("config file not found. Please create a config file at $HOME/.config/gdoc/config.toml"))
	}
	return config
}
